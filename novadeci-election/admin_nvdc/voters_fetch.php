...
if($_REQUEST['action'] == 'fetch_members'){
    ...
    if( !empty($requestData['search']['value']) ) {
      $sql.="where ( pbno LIKE '%".$requestData['search']['value']."%' ";
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        $nestedData['voted_date'] = $row["voted_date"] == "0000-00-00 00:00:00" ? $row["voted_date"] : date('d M, Y - H:i:s', $time);
        $nestedData['comp_name'] = $row["comp_name"];

        $nestedData['action'] = '<button class="btn btn-sm btn-primary editMember" memid="'.$row['id'].'" data-target="#editMemberForm" data-toggle="modal">Edit</button>';
        $data[] = $nestedData;
    }
