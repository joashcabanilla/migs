			...
			<thead class="alert alert-warning">
					...
					<tr class="alert alert-success">
						...
						<?php
								...
								$pbno_label = $memid_label = "";
								if(!empty($fetch['pbno'])){
								   $pbno_label = "<label style='color:black'>PB#: ".$fetch['pbno']."</label>";
								}
								if(!empty($fetch['memid'])){
									$memid_label = "<label style='color:black'>Member ID: ".$fetch['memid']."</label>";
								 }
								 
								 $pb_mem_label = $pbno_label;
								 if(!empty($memid_label)){
									$pb_mem_label = $pbno_label ."<br />". $memid_label;
								 }
									...
									// 	</td>";
										...
										echo "<td style='font-size:25px;'>
											<center>
												...
												<label style='color:green'>  M I G S!  </label>
												<br />
												 <a href='novadeci-election' data-id='".$fetch['id']."' class='voteBtn'>
								 					<label style='font-size:20px;color:purple;cursor:pointer'> >>> Click here to VOTE <<< </label>
												</a>
...
<script>
	...
	$(".voteBtn").click((e) => {
		...
		localStorage.setItem("membersID",id);
